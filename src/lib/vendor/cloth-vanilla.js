/* Cloth — canvas-ui cloth effect.
 *
 * https://canvasui.dev/docs/components/cloth
 */
// @ts-nocheck

/**
 * @typedef {"top" | "bottom" | "left" | "right"} ClothPin
 *
 * @typedef {object} ClothOptions
 * @property {ClothPin} [pin] Edge the cloth hangs from. The opposite side swings free.
 * @property {number} [wind] Wind force driving the fabric (0 lets the waves die down).
 * @property {number} [speed] Playback speed of the cloth motion.
 * @property {number} [amplitude] Height of the fabric folds in CSS pixels.
 * @property {number} [drape] How many CSS pixels the cloth billows toward the viewer on a gust.
 * @property {number} [brush] Strength of the waves the cursor brushes across the fabric (0 disables).
 * @property {number} [brushSize] Radius of the cursor's influence in CSS pixels.
 * @property {number} [damping] How quickly waves settle (higher calms the fabric faster).
 * @property {number} [light] Strength of the directional lighting on the folds (0 to 1).
 * @property {number} [sheen] Strength of the soft sheen on fold crests (0 to 1).
 * @property {number} [shadow] Opacity of the contact shadow under the fabric (0 to 1).
 * @property {number} [cornerRadius] Corner radius of the fabric in CSS pixels.
 * @property {[number, number, number] | "auto"} [backing] Fabric color behind transparent content as RGB in the 0 to 1 range, or "auto" to sample the page background.
 * @property {number} [perspective] Perspective focal length in CSS pixels. Lower exaggerates the 3D depth.
 *
 * @typedef {object} ClothElements
 * @property {HTMLCanvasElement} source Canvas with layoutsubtree that hosts the HTML content.
 * @property {HTMLElement} content The element inside the source canvas that gets captured.
 * @property {HTMLCanvasElement} output Canvas the WebGL effect renders to.
 *
 * @typedef {object} ClothInstance
 * @property {(options: ClothOptions) => void} setOptions Update effect options live.
 * @property {() => void} resize Re-read canvas size. Call when the element is resized.
 * @property {() => void} destroy Stop the loop and release all GPU resources.
 */
...
/**
 * @param {ClothElements} elements
 * @param {ClothOptions} [options]
 * @returns {ClothInstance | null}
 */
export function createCloth(elements, options = {}) {
	...
	function stepSim(dt) {
		...
		for (let b = 0; b < NODES; b++) {
			rowForce[b] =
				Math.sin(kb1 * b - w1 * t + drift) + 0.45 * Math.sin(kb2 * b + w2 * t * 0.8 + 3.0);
		}
		for (let a = 0; a < NODES; a++) {
	...
	function frame(now) {
		...
		if (reducedMotion || (config.wind <= 0.001 && fieldEnergy < 0.004 && touch.s < 0.01)) {
			running = false;
			return;
		}
